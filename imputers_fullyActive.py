...
class OTimputerFullyActive():
    ...
    def fit_transform(self, X, verbose=True, report_interval=500, X_true=None):
        ...
        """
        print("OT imputer vanilla implementation!!")
        X = X.clone()
        n, d = X.shape
        ...
        # Initialize with existing values if present, otherwise use random values
        imps = X[mask.bool()].clone()
        random_init = (self.noise * torch.randn(mask.shape).double() + nanmean(X, 0))[mask.bool()]
        imps[torch.isnan(imps)] = random_init[torch.isnan(imps)]
